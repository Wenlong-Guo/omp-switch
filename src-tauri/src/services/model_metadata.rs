...
pub fn default_builtin_providers() -> Vec<ProviderConfig> {
    vec![
        ...
        ProviderConfig {
            ...
            auth: Some("none".to_string()),
            discovery: Some(DiscoveryConfig {
                discovery_type: "lm-studio".to_string(),
            }),
            model_overrides: None,
            models: None,
            created_at: None,
...
pub fn load_omp_provider_models() -> Vec<ProviderConfig> {
    for executable in omp_executable_candidates() {
        ...
        if let Ok(output) = run_omp_list_models_with_timeout(command, Duration::from_secs(3)) {
...
fn run_omp_list_models_with_timeout(
    mut command: Command,
    timeout: Duration,
    ...
    for raw in output.lines() {
        ...
        by_provider
            ...
            .push(ModelDefinition {
                ...
                context_window: parse_k_number(parts[2]),
                max_tokens: parse_k_number(parts[3]),
                headers: None,
                compat: None,
